		<?
		if(!empty($_GET['n'])){$n = $_GET['n'];}else{$n = 2;}
		$arr = get_data_table($n);    
    if ($arr <> null)
    {
	    ...
	    else
	    {
	       ...
	       foreach ($arr as $key=>$val)
	       {
	          ...
	          $action="/resources/views/ap.php?table=".$table."&num_str=".$n."&count=".$count;
	         ...
	         ?>
					<form class="border" id="<? echo $form_id; ?>" action="<? echo $action; ?>" method="post">
								...
								foreach ($arr_ as $key_=>$val_)
	       				{
			       			...
			       			if($n==$count){if($col_name=="id"){$value = "Нов. >"; $hl_0="";}else{$hl_0="hl_0";}}//Последняя строка //Колонка "id" в последней строке (с номером $count) выглядит, как "Нов. >" (вместо цифр) и без обводки. Все остальные колонки с обводкой.
